...
###################################################
#
# Time average magnetization \overline{M_x}
#
...
for h in np.linspace(0, 2, n):
    
    Hf = -1/j*(np.dot(Jx(j),Jx(j)) + g*np.dot(Jy(j),Jy(j))) - 2*h*Jz(j)
    
    M = np.zeros(len(vect))  #Loschmidt Echo
    ...
    for tt in range(len(vect)):
        ...
        M[tt] = np.dot(np.conjugate(psit), np.dot(Jx(j), psit))
        
    
    
    Mx_mean[i] = (1/max(vect))*np.trapz(M, vect)
    i = i+1

plt.plot(np.linspace(0,3,n), Mx_mean, label=r"$N={}$".format(2*j))
