...
for beta in np.linspace(0, 3, n):
    
    Hf = H0 + beta*(Jz(j) + j*np.identity(dim))
    
    S = np.zeros(len(vect))  #Loschmidt Echo
    ...
    for tt in range(len(vect)):
        ...
        for k in range(dim):
            Ck = np.abs(np.dot(np.conjugate(V0[:,k]), np.dot(U, V0[:,0])))**2
            
            S[tt] = S[tt] - Ck*np.log(Ck)
        
    
    
    S_mean[i] = (1/max(vect))*np.trapz(S, vect)
    i = i+1

plt.plot(np.linspace(0,3,n), S_mean, label=r"$N={}$".format(2*j))
