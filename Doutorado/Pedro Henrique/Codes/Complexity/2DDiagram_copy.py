...
fig, ax = plt.subplots()
ax2 = ax.inset_axes([0.68, 0.21, 0.3, 0.6])
...
for j in [50, 100, 200, 300]:
    ...
    # else:
    C = C[::-1]
    ax.plot(omega_f[1:], C/j, color=c[i], linestyle=l_style[i], label=r"$N={}$".format(2*j))
    # ax.plot(np.linspace(0,1,3), np.ones(3), '--', linewidth = 1.2, color="black")
...
ax.axvline((omega_i+1)/2, linestyle='--', color='black')
...
ax.set_ylim([0, 1.05])
plt.text(0.1, 0.05, r"$(b)$", fontsize=22)
ax.legend(frameon=False,fontsize=16)
plt.tight_layout()

plt.savefig("TimeAveragedComplexity_h0{}.pdf".format(omega_i))
