        ...
        for m in range(dim):  # n=0 (see article)
            if m != 0:
                ...
                raz_Q[i,k] = Q12[i,k]/Q11[i,k]
                    # Termos de R
                        # Derivadas
                d1_inv[i] = np.gradient(inv_sq[i,:])/np.gradient(T)
                d1_raz[i] = np.gradient(raz_Q[i,:])/np.gradient(T)
                d1_g11[i] = np.gradient(Q11[i,:])/np.gradient(T)
                d1_g22[i] = np.gradient(Q22[i,:])/np.gradient(T)
                d1_g12[i] = np.gradient(Q12[i,:])/np.gradient(T)
                d2_inv[k] = np.gradient(inv_sq[:,k])/np.gradient(EY)
                d2_raz[k] = np.gradient(raz_Q[:,k])/np.gradient(EY)
                d2_g11[k] = np.gradient(Q11[:,k])/np.gradient(EY)
                D12_g11[i] = np.gradient(d2_g11[i,:])//np.gradient(EY)
                D11_g22[i] = np.gradient(d1_g22[i,:])/np.gradient(T)
                D21_g12[k] = np.gradient(d1_g12[:,k])/np.gradient(EY)
                D22_g11[k] = np.gradient(d2_g11[:,k])/np.gradient(EY)
                D21_g11[k] = np.gradient(d1_g11[:,k])/np.gradient(EY)

                        # Elementos
                A[i,k] = raz_Q[i,k]*d1_inv[i,k]*d2_g11[i,k] - \
                         d1_inv[i,k]*d1_g22[i,k] + \
                         inv_sq[i,k]*d1_raz[i,k]*d2_g11[i,k] + \
                         inv_sq[i,k]*raz_Q[i,k]*D12_g11[i,k] - \
                         inv_sq[i,k]*D11_g22[i,k]  
                B[i,k] = 2*d2_inv[i,k]*d1_g12[i,k] - \
                         d2_inv[i,k]*d2_g11[i,k] - \
                         raz_Q[i,k]*d2_inv[i,k]*d1_g11[i,k] + \
                         2*inv_sq[i,k]*D21_g12[i,k] - \
                         inv_sq[i,k]*D22_g11[i,k] - \
                         inv_sq[i,k]*d2_raz[i,k]*d1_g11[i,k] - \
                         inv_sq[i,k]*raz_Q[i,k]*D21_g11[i,k]                
                R[i,k] = inv_sq[i,k]*(A[i,k]+B[i,k])
        i = i+1
    k = k+1
...
# ax.plot_wireframe(T,EY,Q11)
# ax.set_title('Q11')

# ax = fig.add_subplot(2, 2, 2, projection='3d')
# ax.plot_surface(T,EY,Q12)
# ax.set_title('Q12')

# ax = fig.add_subplot(2, 2, 3, projection='3d')
# ax.plot_surface(T,EY,Q22)
# ax.set_title('Q12')
