        ...
        for m in range(dim):  # n=0 (see article)
            if m != 0:
                ...
                raz_Q[i,k] = Q12[i,k]/Q11[i,k]
                    # Termos de R
                        # Derivadas
                d1_inv[k] = np.gradient(inv_sq[:,k],T)#/np.gradient(T)
                d1_raz[k] = np.gradient(raz_Q[:,k])#/np.gradient(T)
                d1_g11[k] = np.gradient(Q11[:,k])#/np.gradient(T)
                d1_g22[k] = np.gradient(Q22[:,k])#/np.gradient(T)
                d1_g12[k] = np.gradient(Q12[:,k])#/np.gradient(T)
                d2_inv[i] = np.gradient(inv_sq[i,:])#/np.gradient(EY)
                d2_raz[i] = np.gradient(raz_Q[i,:])#/np.gradient(EY)
                d2_g11[i] = np.gradient(Q11[i,:])#/np.gradient(EY)
                D12_g11[k] = np.gradient(d2_g11[:,k])#/np.gradient(T)
                D11_g22[k] = np.gradient(d1_g22[:,k])#/np.gradient(T)
                D21_g12[i] = np.gradient(d1_g12[i,:])#/np.gradient(EY)
                D22_g11[i] = np.gradient(d2_g11[i,:])#/np.gradient(EY)
                D21_g11[i] = np.gradient(d1_g11[i,:])#/np.gradient(EY)
