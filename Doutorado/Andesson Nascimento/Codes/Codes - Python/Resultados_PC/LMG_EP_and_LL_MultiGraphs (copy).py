...
# This code computes the Bures Angle (LL) and Entropy Production (EP) (lower limit according to the reference PRE, 87,022143 (2013)) to LMG model with hamiltonian 
...
RF = np.zeros(len(vect))
media = np.zeros(len(vect))
...
for h in range(len(hvec)):    
    ...
    for tt in range(len(vect)):
        U = expm(-1j*H*vect[tt])
        L[tt] = round(np.abs(np.dot(dag(V0[:,0]), np.dot(U, V0[:,0])))**2,13) 
        LL[tt] = mt.acos(L[tt])
        if L[tt] <= 0:
            L[tt] = 1e-16
    ...
    plt.plot(vect,EP,label='h={}'.format(hvec[h])) # Plotting # choose the graphi
