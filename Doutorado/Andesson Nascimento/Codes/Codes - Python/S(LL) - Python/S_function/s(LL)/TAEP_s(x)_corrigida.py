...
dim = 150 # Dimension of time vector
dimh = 200 # Dimension of h
...
#     # Retirar o problema da divizão por zero
#     if divisor != 0:
#       resultado = - np.log(divisor)
#     else:
#       resultado = 0  # ou algum valor padrão adequado à sua aplicação
    ...
    for tt in range(len(vect)):
        ...
        # Retirar problema dos zeros
            if (1-rvec[rr]+(2/mt.pi)*LL[tt])/(1-rvec[rr]) > 0 and (rvec[rr]-(2/mt.pi)*LL[tt])/rvec[rr] > 0:
                s_exac[tt,rr] = ((rvec[rr]-(2/mt.pi)*LL[tt])*np.log((rvec[rr]-(2/mt.pi)*LL[tt])/rvec[rr]) +\
                                 + (1-rvec[rr]+(2/mt.pi)*LL[tt])*np.log((1-rvec[rr]+(2/mt.pi)*LL[tt])/(1-rvec[rr]))) 
            else:
                s_exac[tt,rr] = 0             
...
plt.plot(hvec,TAEP_lb,label='lower')
plt.plot(hvec,TAEP_exac,label='s(x)')
plt.axvline(0.5, color="red",linestyle='--')
plt.ylabel(r'$\overline{s\left(\frac{2}{\pi}\mathcal{L} \right)}$', fontsize=14)
plt.xlabel('h', fontsize=14)
plt.legend()

np.savetxt(os.path.join(os.getcwd(),'TAEP_exac_j={}2.txt'.format(j)), TAEP_exac)
np.savetxt(os.path.join(os.getcwd(),'TAEP_lb_j={}2.txt'.format(j)), TAEP_lb)
np.savetxt(os.path.join(os.getcwd(),'TAEP_ub_j={}2.txt'.format(j)), TAEP_ub)
