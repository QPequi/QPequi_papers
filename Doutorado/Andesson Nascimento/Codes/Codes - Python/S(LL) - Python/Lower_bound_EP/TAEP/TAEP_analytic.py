...
#Final hamiltonian
    ...
    TAEP[h] = (8/np.pi**2)*(np.arccos(np.exp(-N*RF[tt]/2)))**2
...
np.savetxt(os.path.join(os.getcwd(),'Derivada_Analitica_j={}(h04-06_50pontos).txt').format(j), DerT)
...
plt.savefig('DerTAEP_anal_j={}.svg'.format(j), format='svg')

# Fim da contagem de tempo
end_time = time.time()

# Tempo total de execução
total_time = (end_time - start_time)*0.0167

# Imprime o tempo total em segundos
print(f"Tempo total de execução: {total_time} minutos")
