...
h = 1
H = -(1/j)*(np.dot(Jx(j),Jx(j)) + g*np.dot(Jy(j),Jy(j))) - 2*h*Jz(j) #- 2*hvec[h]*Jz(j)
...
plt.plot(vect,EP,label='h=1')
# plt.plot(vect,EP,label='h={}'.format(hvec[h])) # Plotting # choose the graphic
